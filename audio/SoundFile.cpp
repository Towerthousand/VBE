...
bool SoundFile::openRead(const std::string& filename) {
    ...
    if (!m_file)
    {
		VBE_ASSERT(m_file, "Failed to open sound file \"" << filename << "\" (" << sf_strerror(m_file) << ")");
        return false;
    }
...
bool SoundFile::openRead(const void* data, std::size_t sizeInBytes) {
	...
	if (!m_file) {
		VBE_ASSERT(m_file, "Failed to open sound file from memory (" << sf_strerror(m_file) << ")");
        return false;
    }
...
bool SoundFile::openRead(InputStream& stream) {
	...
	if (!m_file) {
		VBE_ASSERT( m_file, "Failed to open sound file from stream (" << sf_strerror(m_file) << ")");
        return false;
    }
...
bool SoundFile::openWrite(const std::string& filename, unsigned int channelCount, unsigned int sampleRate) {
	...
	if (format == -1) {
		...
		VBE_ASSERT( m_file, "Failed to create sound file \"" << filename << "\" (unknown format)");
        return false;
    }
	...
	if (!m_file) {
		VBE_ASSERT( m_file, "Failed to create sound file \"" << filename << "\" (" << sf_strerror(m_file) << ")");
        return false;
    }
...
std::size_t SoundFile::read(short* data, std::size_t sampleCount) {
...
void SoundFile::write(const short* data, std::size_t sampleCount) {
	if (m_file && data && sampleCount) {
...
void SoundFile::seek(float timeOffset) {
	if (m_file) {
		sf_count_t frameOffset = static_cast<sf_count_t>(timeOffset * m_sampleRate);
...
sf_count_t SoundFile::Stream::read(void* ptr, sf_count_t count, void* userData) {
    Stream* stream = static_cast<Stream*>(userData);
	long int position = stream->source->tell();
	if (position != -1) {
