...
bool ShaderProgram::makeProgram(const std::string& vp_filename, const std::string& gp_filename, const std::string& fp_filename) {
	//LOAD AND COMPILE VERTEX SHADER
	VBE_DLOG("* Loading new vertex shader from " << vp_filename );
	Shader vertex(GL_VERTEX_SHADER);
	if(!vertex.load(vp_filename))
	...
	VBE_DLOG("* Loading new geometry shader from " << gp_filename );
	Shader geometry(GL_GEOMETRY_SHADER);
	if(!geometry.load(gp_filename))
		return false;
	if (!geometry.compile()) {
		geometry.printInfoLog();
		VBE_LOG("#ERROR Compile failed for geometry shader '" << gp_filename << "'." );
		return false;
	}
	else {VBE_DLOG( " - Compiled " << gp_filename << " successfully." );}
	...
	else {VBE_DLOG( " - Linked " << vp_filename << " and " << gp_filename << " and " << fp_filename << " successfully. PROGRAMID: " << programHandle );}
